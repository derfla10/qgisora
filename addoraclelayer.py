#########################################################
# Program : addoraclelayer.py                           #
# Author  : Alfred de Jager                             #
# Version : 2.0                                         #
# Purpose : Makes a map of an oracle spatial table      #
#           using predefined colorscheme's if available #
# Date    : 4 July 2025                                 #
...
oraconn = oracledb.connect(user=myusername,password=mypassword, dsn=myhost+":"+myport+"/"+mydb)
...
if (query_identifierrow == "" ):
	...
	elif(mytable_name == "GRID_025DD_LANDCOVER" or mytable_name == "GRID_025DD_TEMPERATURE_STATS" or mytable_name == "GRID_025DD_TEMPERATURE_WAVES"):
	...
	if not mydblayer.isValid():
		...
		if (timeselect):
			...
			if (minmaxcurrow[0][1] == None):
				...
				testlist = ["TEMP","RAIN","SPI","ANOMALY","ABSORBED","SMA"]
				...
				elif (mytype == "SPI" or mytype == "ANOMALY" or mytype == "SMA"):
